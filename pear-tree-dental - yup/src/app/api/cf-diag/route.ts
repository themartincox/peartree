...
import { SERVICE_TYPE, LOCATION_TYPE, BLOG_TEMPLATE_TYPE } from '@/lib/contentTypes' // Changed TEMPLATE_TYPE to BLOG_TEMPLATE_TYPE
...
export async function GET() {
  ...
  const [typesRes, serviceRes, locationRes, templateRes] = await Promise.all([
    ...
    fetchJSON(`${base}/entries?content_type=${encodeURIComponent(BLOG_TEMPLATE_TYPE)}&limit=1`, token), // Changed TEMPLATE_TYPE to BLOG_TEMPLATE_TYPE
  ])
  ...
  return NextResponse.json({
    ...
    template: { typeId: BLOG_TEMPLATE_TYPE, ...summarise(templateRes) }, // Changed TEMPLATE_TYPE to BLOG_TEMPLATE_TYPE
  })
}