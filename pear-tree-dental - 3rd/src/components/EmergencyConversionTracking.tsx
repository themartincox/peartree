...
export default function EmergencyConversionTracking() {
  ...
  useEffect(() => {
    ...
    const setupClickTracking = () => {
      ...
      const emergencyPhonePrimary = document.getElementById('emergency-phone-primary');
      const emergencyPhoneConditions = document.getElementById('emergency-phone-conditions');
      const emergencyPhoneFinal = document.getElementById('emergency-phone-final');

      // Secondary CTAs
      const emergencyDirections = document.getElementById('emergency-directions');
      const emergencyBookOnline = document.getElementById('emergency-book-online');
      const emergencyMembership = document.getElementById('emergency-membership');

      // Track primary phone clicks
      [emergencyPhonePrimary, emergencyPhoneConditions, emergencyPhoneFinal].forEach((element, index) => {
        if (element) {
          element.addEventListener('click', () => {
            trackPhoneClick(`emergency_phone_${['primary', 'conditions', 'final'][index]}`, '0115 931 2935');
      ...
      if (emergencyMembership) {
        emergencyMembership.addEventListener('click', () => {
          trackLocationConversion('emergency_prevention_interest', 'burton_joyce');
        });
      }
    };
  ...
  }, [trackServiceInterest, trackPhoneClick, trackBookingAttempt, trackLocationConversion]);
