  ...
  try {
    ...
    try {
      ...
      testResults.push({
        ...
        details: {
          ...
          accessible: homepageResponse.ok,
        },
      });
    } catch (error) {
      testResults.push({
    ...
    try {
      const membershipResponse = await fetch(
        `${process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000"}/membership`,
      );
      testResults.push({
        ...
        details: {
          ...
          accessible: membershipResponse.ok,
        },
      });
    } catch (error) {
      testResults.push({
    ...
    try {
      const signupResponse = await fetch(
        `${process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000"}/membership/signup`,
      );
      testResults.push({
    ...
    try {
      const successResponse = await fetch(
        `${process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000"}/membership/signup/success?applicationId=TEST-123&plan=COMPLETE%20CARE&name=Test%20User`,
      );
      testResults.push({
    ...
    for (const route of apiRoutes) {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000"}${route.path}`,
        );
    ...
    testResults.push({
      ...
      details: {
        workingRoutes: apiRoutesWorking,
