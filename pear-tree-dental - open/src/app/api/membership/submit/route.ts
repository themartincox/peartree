...
const saveApplicationToFile = async (submission: MembershipSubmission) => {
  try {
    ...
    if (
      ...
      console.log(
        "🌐 Serverless environment detected - file saving may not be available",
      );
    }

    console.log(
      "Saving application to file for:",
    ...
    console.log(
      "Total applications after adding new one:",
      applications.length,
    );
    ...
    if (fs.existsSync(APPLICATIONS_FILE)) {
      ...
      console.log(
        "✅ File verification successful, applications count:",
        verifyApplications.length,
      );
    }
    ...
    console.warn(
      "⚠️ Failed to save application to file (expected on serverless):",
    ...
    if (process.env.NODE_ENV === "development") {
      ...
      console.error(
        "❌ Directory exists:",
        fs.existsSync(path.dirname(APPLICATIONS_FILE)),
      );
...
const getPartnerDentistName = (formData: Record<string, unknown>): string => {
  ...
  if (
    ...
    formData.partnerDentistGenderPreference
  ) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(
      ...
      securityValidation.rateLimitResult.remaining,
    );
    ...
    try {
      ...
      emailError =
        emailErr instanceof Error ? emailErr.message : "Email service error";
      emailSent = false;
