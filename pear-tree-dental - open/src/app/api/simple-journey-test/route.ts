  ...
  try {
    ...
    const membershipPlans = {
      planA: {
        ...
        price: "£10.95",
        dailyCost: "36p per day",
      },
      planB: {
        name: "ROUTINE CARE",
        price: "£15.95",
        dailyCost: "53p per day",
      },
      planC: {
        name: "COMPLETE CARE",
        price: "£19.95",
        dailyCost: "66p per day",
      },
      planD: {
        name: "COMPLETE CARE PLUS",
        price: "£25.95",
        dailyCost: "86p per day",
      },
      planE: {
        name: "PERIODONTAL HEALTH",
        price: "£29.95",
        dailyCost: "99p per day",
      },
      family: {
        name: "FAMILY PLAN",
        price: "£49.50",
        dailyCost: "£1.65 per day",
      },
    };
    ...
    testResults.push({
      ...
      message: validationResult.isValid
        ? "Valid form data passes all security checks"
        : "Valid data incorrectly rejected",
      details: {
    ...
    const isXssSanitized =
      !JSON.stringify(xssValidation.sanitizedData).includes("<script>") &&
      !JSON.stringify(xssValidation.sanitizedData).includes("<img");

    testResults.push({
      step: "3. XSS Security Protection",
      status: isXssSanitized ? "PASS" : "FAIL",
      message: isXssSanitized
        ? "XSS attempts successfully blocked and sanitized"
    ...
    const hasAppropriateErrors =
      Object.keys(incompleteValidation.errors).length > 0;

    testResults.push({
      step: "4. Required Field Validation",
      status: hasAppropriateErrors ? "PASS" : "FAIL",
      message: hasAppropriateErrors
    ...
    testResults.push({
      ...
      message: familyValidation.isValid
        ? "Family plan data structure validated successfully"
        : "Family plan validation failed",
      details: {
    ...
    const criticalEnvMissing =
      !envCheck.hasEmailUser || !envCheck.hasEmailPassword;

    testResults.push({
      step: "7. Environment Configuration",
      status: criticalEnvMissing ? "WARNING" : "PASS",
      message: criticalEnvMissing
        ? "Email environment variables missing (development environment)"
...
function generateRecommendations(testResults: any[]): string[] {
  ...
  if (failed.length === 0 && warnings.length === 0) {
    ...
    recommendations.push(
      "📧 Configure email environment variables for complete functionality",
    );
  }
  ...
  if (warnings.length > 0) {
    recommendations.push(
      `🟡 Attention: ${warnings.length} configuration warnings`,
    );
    warnings.forEach((w) =>
      recommendations.push(`  - ${w.step}: ${w.message}`),
    );
  }
