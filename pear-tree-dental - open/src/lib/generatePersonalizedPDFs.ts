...
export const generateDirectDebitGuaranteePDF = async (
  ...
  doc.text("DIRECT DEBIT GUARANTEE", pageWidth / 2, yPosition, {
    align: "center",
  });
  ...
  doc.text("Pear Tree Dental Centre", pageWidth / 2, yPosition, {
    align: "center",
  });
  ...
  doc.setFont("helvetica", "normal");
  doc.text(
    `Main Account Holder: ${patientInfo.firstName} ${patientInfo.lastName}`,
    margin,
    yPosition,
  );
  yPosition += 8;
  ...
  if (
    patientInfo.isFamily &&
    patientInfo.partnerFirstName &&
    patientInfo.partnerLastName
  ) {
    doc.text(
      `Partner: ${patientInfo.partnerFirstName} ${patientInfo.partnerLastName}`,
      margin,
      yPosition,
    );
    yPosition += 8;
  ...
  doc.text(
    `Membership Plan: ${patientInfo.planName} - ${patientInfo.planPrice}/month`,
    margin,
    yPosition,
  );
  yPosition += 8;
  ...
  if (patientInfo.accountHolderName) {
    doc.text(
      `Direct Debit Account Holder: ${patientInfo.accountHolderName}`,
      margin,
      yPosition,
    );
    yPosition += 8;
  ...
  if (patientInfo.sortCode && patientInfo.accountNumber) {
    doc.text(
      `Account Details: Sort Code ${patientInfo.sortCode}, Account ****${patientInfo.accountNumber.slice(-4)}`,
      margin,
      yPosition,
    );
    yPosition += 8;
  ...
  guaranteePoints.forEach((point, _index) => {
  ...
  doc.setFont("helvetica", "normal");
  doc.text(
    "Membership Plans Ltd trading as Lloyd & Whyte Flexiplan is registered in England No 06322047.",
    margin,
    yPosition,
  );
  yPosition += 8;
  ...
  doc.setFontSize(10);
  doc.text(
    "Pear Tree Dental Centre | 22 Nottingham Rd, Burton Joyce, Nottingham NG14 5AE | 0115 931 2935",
    pageWidth / 2,
    yPosition,
    { align: "center" },
  );
...
export const generateMembershipTermsPDF = async (
  patientInfo: PatientInfo,
): Promise<Blob> => {
  if (typeof window === "undefined") {
    throw new Error("PDF generation only available on client side");
  ...
  doc.setFontSize(18);
  doc.setFont("helvetica", "bold");
  doc.text("MEMBERSHIP TERMS AND CONDITIONS", pageWidth / 2, yPosition, {
    align: "center",
  });
  yPosition += 12;
  ...
  if (patientInfo.dentistName) {
    addTextWithPageBreaks(
      `Assigned Dentist: ${patientInfo.dentistName}`,
      false,
      10,
    );
    yPosition += 5;
  ...
  if (patientInfo.isFamily && patientInfo.partnerDentistName) {
    addTextWithPageBreaks(
      `Partner's Assigned Dentist: ${patientInfo.partnerDentistName}`,
      false,
      10,
    );
    yPosition += 5;
