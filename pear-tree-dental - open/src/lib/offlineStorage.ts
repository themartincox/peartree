...
class OfflineStorageManager {
  ...
  async storeRequest(
    ...
    return new Promise((resolve, reject) => {
      ...
      addRequest.onerror = () => {
        console.error(
          `[OfflineStorage] Failed to store ${type} request:`,
          addRequest.error,
        );
  ...
  async getStoredRequests(
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () => {
        console.error(
          "[OfflineStorage] Failed to get stored requests:",
          request.error,
        );
        reject(request.error);
      };
  ...
  async removeRequest(id: string): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      deleteRequest.onerror = () => {
        console.error(
          "[OfflineStorage] Failed to remove request:",
          deleteRequest.error,
        );
...
export async function storeContactForm(
...
export async function storeMembershipSignup(
    ...
    for (const request of requests) {
      try {
        ...
        if (response.ok) {
          ...
          console.log(
            `[OfflineStorage] Successfully synced ${request.type} request`,
          );
        } else {
        ...
        console.error(
          `[OfflineStorage] Failed to sync ${request.type} request:`,
          error,
        );
  ...
  const submitForm = async (
    ...
    if (isOnline) {
      ...
      try {
        ...
        console.warn(
          "[OfflineStorage] Direct submission failed, storing offline:",
          error,
        );
      }
    }
