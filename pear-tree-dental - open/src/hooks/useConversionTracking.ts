...
export const useConversionTracking = (): {
  ...
  trackPhoneClick: (location: string, phoneNumber: string) => void;
  trackServiceInterest: (
    service: string,
    serviceType?: "emergency" | "cosmetic" | "general" | "orthodontics",
  ) => void;
  trackMembershipPlanSelect: (planName: string, monthlyValue: number) => void;
  ...
  const trackConversion = useCallback(
    (eventType: string, customMetadata: Record<string, any> = {}) => {
      incrementClickCount();
    ...
    [
      ...
      metrics,
      sessionId,
      incrementClickCount, // Send to analytics (multiple providers for redundancy)
      sendToAnalytics, // Local storage for offline resilience
      storeLocalConversion,
    ],
  );
  ...
  const trackMembershipPlanSelect = useCallback(
    (planName: string, monthlyValue: number) => {
  ...
  const trackLocationConversion = useCallback(
    (actionType: string, targetLocation = "burton_joyce") => {
  ...
  const trackBookingAttempt = useCallback(
    (source: string, formType: "contact" | "booking" = "booking") => {
