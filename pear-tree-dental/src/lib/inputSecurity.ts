...
const UK_PHONE_REGEX = /^(?:(?:\+44\s?)|(?:0))(?:\d{2}\s?\d{4}\s?\d{4}|\d{3}\s?\d{3}\s?\d{4}|\d{4}\s?\d{6}|\d{5}\s?\d{5})$/;
 ...
 */
export function validateAndSanitizePhone(phone: string): { isValid: boolean; sanitized: string; error?: string } {
 ...
 */
export function validateAndSanitizePostcode(postcode: string): { isValid: boolean; sanitized: string; error?: string } {
  ...
  if (!UK_POSTCODE_REGEX.test(sanitized)) {
    return { isValid: false, sanitized: '', error: 'Invalid UK postcode format' };
  }
 ...
 */
export function validateAndSanitizeSortCode(sortCode: string): { isValid: boolean; sanitized: string; error?: string } {
  ...
  if (!SORT_CODE_REGEX.test(sanitized)) {
    return { isValid: false, sanitized: '', error: 'Invalid sort code format (should be XX-XX-XX)' };
  }
 ...
 */
export function validateAndSanitizeAccountNumber(accountNumber: string): { isValid: boolean; sanitized: string; error?: string } {
  if (!accountNumber || typeof accountNumber !== 'string') {
    return { isValid: false, sanitized: '', error: 'Account number is required' };
  }
  ...
  if (!ACCOUNT_NUMBER_REGEX.test(sanitized)) {
    return { isValid: false, sanitized: '', error: 'Invalid account number (should be 6-8 digits)' };
  }
 ...
 */
export function validateAndSanitizeName(name: string, fieldName = 'Name'): { isValid: boolean; sanitized: string; error?: string } {
  ...
  if (sanitized.length > 50) {
    return { isValid: false, sanitized: '', error: `${fieldName} is too long (max 50 characters)` };
  }
 ...
 */
export function validateMembershipForm(data: any): ValidationResult {
  ...
  const accountNumberResult = validateAndSanitizeAccountNumber(data.accountNumber);
  ...
  const accountHolderResult = validateAndSanitizeName(data.accountHolderName, 'Account holder name');
  ...
  // Copy boolean fields
  const booleanFields = ['directDebitConfirmed', 'membershipTermsRead', 'marketingConsent'];
