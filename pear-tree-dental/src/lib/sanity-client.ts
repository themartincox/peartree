...
interface ImageUrlBuilder {
  image: (
    source: SanityImage | SanityImageAsset | { asset: SanityImageAsset },
  ) => {
    url: () => string;
...
try {
  ...
  const nextSanity = require("next-sanity") as {
    createClient: (config: unknown) => SanityClient;
  };
  const imageUrlBuilderLib = require("@sanity/image-url") as (
    client: SanityClient,
  ) => ImageUrlBuilder;
...
export async function getBlogPost(slug: string): Promise<BlogPost | null> {
  ...
  try {
    return client.fetch(queries.blogPostBySlug, {
      slug,
...
export async function getHomepageContent(): Promise<HomepageContent | null> {
  ...
  try {
    return client.fetch(
      queries.homepageContent,
    ) as Promise<HomepageContent | null>;
  } catch {
    return null;
  }
}
