...
const run = <T>(ctx: string, call: () => Promise<T>) =>
  limiter(async () => {
    ...
    if (flags.USE_MOCK || flags.FAST_BUILD) {
      // @ts-expect-error – callers expect shape { items: [] } or a single entry
      return Array.isArray(undefined) ? [] : ({ items: [] } as T)
    }
    try {
      return await call()
    } catch (_err) {
...
export const cfEntries = <T>(q: any, ctx = 'entries') =>
  run<T>(ctx, () => client!.getEntries<T>(q))
...
// ────────────────────────────────────────────────────────────────────────────
// Domain helpers (kept, now fail-soft in mock/fast modes)
// ────────────────────────────────────────────────────────────────────────────
...
export async function fetchPriorityLocations() {
  const prim = await cfEntries<any>(
    { content_type: process.env.CONTENTFUL_LOCATION_TYPE_ID || 'location', 'fields.priority': true, limit: 100 },
