...
const loadApplicationsFromFile = (): MembershipSubmission[] => {
  try {
    ...
    console.log(
      "Loaded applications count:",
      Array.isArray(applications) ? applications.length : "not an array",
    );
...
const convertToCSV = (applications: MembershipSubmission[]): string => {
  ...
  const rows = applications.map((app) => [
    ...
    (app as any).sortCode || "",
    (app as any).accountNumber
      ? `****${(app as any).accountNumber.slice(-4)}`
      : "",
    (app as any).partnerFirstName || "",
    (app as any).partnerLastName || "",
    (app as any).partnerEmail || "",
    (app as any).staffMemberName || "",
    (app as any).familyMembers
      ? JSON.stringify((app as any).familyMembers).length
      : "0",
  ]);
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (isServerless) {
      console.log(
        "🌐 Serverless environment detected - file storage not available",
      );
        ...
        return new NextResponse(csvNotice, {
          headers: {
            ...
            "Content-Disposition": download
              ? `attachment; filename="serverless-notice-${new Date().toISOString().split("T")[0]}.csv"`
              : "inline",
          },
        });
      }
