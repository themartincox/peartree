...
export default function MembershipApplicationsAdmin() {
  ...
  const testDirectEndpoint = async () => {
    try {
      ...
      console.log('Submit endpoint response:', response.status, response.statusText);
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <div className="mb-6">
        <h1 className="text-3xl font-bold mb-2">Membership Applications Admin</h1>
      ...
      {!loading && !error && applications.length === 0 && (
        <div className="p-8 bg-yellow-100 rounded-lg text-center">
          ...
          <p className="text-yellow-700 mt-2">
            No membership applications have been submitted yet, or they may not be saving properly.
          </p>
              ...
              <div key={app.id || index} className="p-6 border rounded-lg bg-white shadow-sm hover:shadow-md transition-shadow">
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    {app.staffMemberName && (
                      <p className="text-sm">
                        <span className="font-medium">Staff:</span> {app.staffMemberName}
                      </p>
                    )}
                    {app.isExistingPatient && (
                      <p className="text-sm">
                        <span className="font-medium">Existing Patient:</span> {app.isExistingPatient}
                      </p>
                    )}
                  </div>
                </div>
