...
export default function FamilyLocationLayout({ data }: FamilyLocationLayoutProps) {
        ...
        <div className="relative container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-4xl mx-auto text-center">
            <div className="flex items-center justify-center space-x-2 mb-6">
              <Icon name="Users" className="h-6 w-6 text-dental-green" />
              <span className="text-dental-green font-medium">{hero.badge}</span>
            </div>
            <h1 className="text-4xl md:text-6xl font-bold mb-6 text-white">
              {hero.title}
              <span className="text-dental-green block">{hero.subtitle}</span>
            </h1>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <Button asChild variant="outline" size="lg" className="border-dental-green text-dental-green hover:bg-dental-green hover:text-dental-navy px-8 py-4">
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid md:grid-cols-3 gap-8">
              {access.points.map((point, index) => (
                <Card key={index} className="p-6 border border-dental-green/20 bg-gradient-to-br from-white to-dental-green/5">
                  <CardHeader className="text-center pb-4">
                    <Icon name={point.icon as keyof typeof LucideIcons} className="h-12 w-12 mx-auto text-dental-green mb-4" />
                    <CardTitle className="text-xl">{point.title}</CardTitle>
                  </CardHeader>
                  <CardContent className="text-center">
                    <p className="text-2xl font-bold text-dental-green mb-2">{point.value}</p>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        <Icon name={service.icon as keyof typeof LucideIcons} className="h-8 w-8 text-dental-green flex-shrink-0" />
                        <div>
                          <h3 className="text-xl font-semibold">{service.title}</h3>
                          {service.badge && <Badge variant="secondary" className="text-xs mt-1 bg-dental-green/10 text-dental-green">{service.badge}</Badge>}
                        </div>
                      </div>
                      {service.price && <div className="text-right"><p className="font-bold text-dental-green">{service.price}</p></div>}
                    </div>
            ...
            <div className="grid md:grid-cols-2 gap-12">
              <div>
                ...
                <div className="space-y-6">
                  <div className="flex items-start space-x-4">
                    <Icon name="MapPin" className="h-6 w-6 text-dental-green mt-1 flex-shrink-0" />
                    <div>
                  ...
                  <div className="flex items-start space-x-4">
                    <Icon name="Phone" className="h-6 w-6 text-dental-green mt-1 flex-shrink-0" />
                    <div>
              ...
              <div>
                ...
                <div className="bg-white p-6 rounded-lg shadow-lg border border-dental-green/20">
                  <h3 className="font-semibold mb-4 text-dental-green">{directions.routeDescription}</h3>
                  <ol className="space-y-2 text-gray-700">
                    {directions.steps.map((step, index) => (
                      <li key={index} className="flex items-start space-x-2">
                        <span className="bg-dental-green text-white rounded-full w-5 h-5 flex items-center justify-center text-xs font-semibold mt-0.5 flex-shrink-0">{index + 1}</span>
                        <span>{step}</span>
                      </li>
                    ))}
                  </ol>
      ...
      <section className="py-16 bg-gradient-to-r from-dental-green to-pear-gold text-white">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Button asChild size="lg" className="bg-white hover:bg-white/90 text-dental-green font-semibold px-8 py-4">
                <Link href={`tel:${practiceInfo.contact.phoneInternational}`}>
                  <Icon name="Phone" className="h-5 w-5 mr-2" />
                  Call: {practiceInfo.contact.phone}
                </Link>
              </Button>
              <Button asChild variant="outline" size="lg" className="border-white text-white hover:bg-white hover:text-dental-green px-8 py-4">
                <Link href="/membership">
                  <Icon name="Users" className="h-5 w-5 mr-2" />
                  Family Plans
                </Link>
