...
export const SUPPORTED_LOCATIONS: { [key: string]: LocationInfo } = {
  ...
  'lowdham': {
    ...
    route: 'via A612',
    pageUrl: '/ng14-dental',
    features: ['Quick local journey', 'Same postal area', 'Village convenience']
  },
  'calverton': {
    area: 'Calverton',
    postcode: 'NG14',
...
export function detectLocationFromString(input: string): LocationInfo | null {
  ...
  // Partial match
  for (const [key, location] of Object.entries(SUPPORTED_LOCATIONS)) {
    if (location.area.toLowerCase().includes(normalizedInput) ||
...
export function suggestLocations(userInput: string): LocationSuggestion[] {
  ...
  for (const [key, location] of Object.entries(SUPPORTED_LOCATIONS)) {
    ...
    // Partial area match
    else if (location.area.toLowerCase().includes(input) || input.includes(location.area.toLowerCase())) {
      confidence = 70;
