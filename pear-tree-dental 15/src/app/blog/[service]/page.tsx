...
export async function generateMetadata({ params }: { params: { service: string } }): Promise<Metadata> {
  const post = await fetchBlogPostBySlug(params.service);

  if (!post) {
    return {
      title: 'Blog Post Not Found | Pear Tree Dental',
  ...
  const canonicalUrl = seoFields?.canonicalUrl || `https://peartree.dental/blog/${params.service}`;

  return {
    title: `${pageTitle} | Pear Tree Dental Blog`,
...
export default async function BlogPostPage({ params }: { params: { service: string } }) {
  const post = await fetchBlogPostBySlug(params.service);
